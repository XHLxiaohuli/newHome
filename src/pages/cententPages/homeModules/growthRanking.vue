  ...
  data() {
    return {
      ...
      wechatList_searchVal:{
          ...
          fill:[
            { type:'input',  s_name:'微信名',  prompt:'微信名 / 微信号'  },
            {
      ...
      wechatListFormVal:{title:[
        { name:'微信号',  type:'string',    tag:'wxh',    width:30  },
        { name:'微信昵称',type:'string',    tag:'szfz',    width:30  },
        { name:'新增好友数量',  type:'string',    tag:'sfrz',     width:30  },
      ],value:[]},
      // 最小时间条件
      最小时间:'',
      // 最大时间条件
  ...
  methods:{
    ...
    wechatListValFun:function(obj={
      ...
      getIticket(function(ticket){
        this_.$axios.get(
          "http://"+api+"/api/1.0/statistic/page/friend"+'?pageNum='+obj.pageNum+'&pageSize='+obj.pageSize+ '&type='+obj.type+'&private=' +obj.private +'&maxConversationTime='+obj.maxConversationTime+'&minConversationTime='+obj.minConversationTime,
          {headers:{
