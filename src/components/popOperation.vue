        ...
        <div style="width:100%;height:auto;">
          ...
          <div class="formFillOut" :style="labelCard && labelCard==true ? 'width: 800px;' : 'width: 660px;'">
            <div class="formFillOut_c" v-for="(v,i) in formFillOutVal" :key="i" :data-type="v.type">
              ...
              <!-- 标签 -->
              <div class="labelCard_s"  v-if="v.type=='labelCard'">
                <div class="labelCard_d" v-for="(val_,i) in v.value" :key="i" v-if="v.label=='string'">
                  {{val_.val}}
                  <img :data-id="val_.id" src="../assets/img/删除.png" class="del_img2">
                </div>
                <div class="labelCard_div" v-for="(val_,i) in v.value" :key="i" v-if="v.label=='object'">
                  <div class="labelCard_obj">
                    <img :src="val_.url" class="labelCard_img" alt="" v-if="val_.url">
                    <div class="labelCard_val">
                      <p v-for="(text,i) in val_.valList" :key="i">{{text}}</p>
                    </div>
                  </div>
                  <img :data-id="val_.id" src="../assets/img/删除.png" class="del_img2">
                </div>
...
export default {
  ...
  components: {  maskPopModule,multiSelect  },
  watch:{
    popOperaVal:{//深度监听对象的变化
      handler(newVal, oldVal){
        this.formFillOutVal=newVal;

        var this_ = this;
        this_.liandong_ = false;
        setTimeout(function(){  this_.liandong_ = true  },20);
