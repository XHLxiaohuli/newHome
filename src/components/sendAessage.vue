<template>
  <div class="judgment">
    <p  v-if="type !== '10000'"  
        :style="boolobj?'text-align: right;':'text-align: left;'">
        <span>{{getdate(time)}}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span>{{name}}</span>
    </p>
    <div v-if="type!=='10000'" :class="boolobj ? 'sendAessage_' : 'closedAessage_'">
      <img :src="boolobj ? topUrl[0]:topUrl[1]" alt=""/>
...
<script>
export default {
  props:['boolobj','type','msgVal','time','topUrl','name'],
  data() {
    return {
      showMsg:''
    }
