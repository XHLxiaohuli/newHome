<template>
  <div class="chatRecord">
    <maskPopModule>
    <div :style="listBool && 'width:1000px;height650px;'">
      <div class="layout_left_" v-if="listBool">
        <clientList
          :type="chatRecordType"
          :unesArr="unesArr"
          :groupNameShow="groupNameShow"
          @switchId="switchIdFun"
          @switchWatchId="switchWatchIdFun"
        />
      </div>
      <div class="chatRecord_div" :style="!listBool && 'border-radius: 5px;'">
        <!-- 头部 -->
        <div class="chatRecord_top">
          <span>聊天记录</span>
          <img src="../assets/img/close.png" alt="" @click="hideComponent">
        </div>
        <!-- 聊天记录搜索区 -->
          ...
          <div class="input_search">
            ...
            <div class="input_seaChenr" v-if="msgValTis">
              <div  class="msgVal_pipei" 
                ...
                {{v.msgVal}}
                <p><span>{{getdate(v.time)}}</span><span>{{v.id}}</span></p>
              </div>
            </div>
          </div>
...
export default {
  props:['listBool','id','wechatid','wxid','topUrl','chatRecordType'],
  data() {
    return {
      // 时间筛选组件
      searchModuleVal:{
  ...
  components: {  maskPopModule,searchModule,sendAessage,clientList  },
  mounted() {
    // this.judgmentValFun();
    this.rollingFUN();

    // 初始获取聊天数据
    if(this.chatRecordType == 'wechatId'){
      this.groupNameShow = false;
      this.wechatValFun();
    }else if(this.chatRecordType == 'sellerId'){
      this.sellerValFun();
    }else{  this.clickBtn();  }
  ...
  methods:{
    ...
    // 获取微信数据
    wechatValFun:function(){
      var this_ = this;
      getIticket(function(ticket){
        this_.$axios.get(
          "http://"+api+"/api/1.0/groups/wechatFriend/find/"+this_.id,
          {headers:{
            'content-type': 'application/x-www-form-urlencoded',
            'ticket':ticket
          }}
        ...
        .then((response)=>{
          if(response.data.success){
            this_.unesArr = response.data.result;
          }else{
            alert(response.data.msg);
          };          
        })
    ...
    sellerValFun:function(){
      var this_ = this;
      getIticket(function(ticket){
        this_.$axios.get(
          "http://"+api+"/api/1.0/groups/wechatFriend/find/sellerId"+"?sellerId="+this_.id,
          {headers:{
            'content-type': 'application/x-www-form-urlencoded',
            'ticket':ticket
          }}
    ...
    // 
    switchWatchIdFun:function(data){
      if(!data){return;}
      this.topUrl_=[];
      // 获取头像
      this.topUrl_[0] = data.headImgUrl;
    },
    // 切换聊天数据用户id
    switchIdFun:function(data){
      var this_ = this;
      this.msgValList=[];
      // 模糊搜索id
      this.MHid='';
      ...
      // 获取聊天消息参数
      this.wechatid_=data.wechatId;
      // 获取聊天信息参数
      this.wxid_=data.wxid;
      // 获取头像
      this.topUrl_[1] = data.headImgUrl;
